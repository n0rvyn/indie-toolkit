...
Same shape as the inline `model:` finding: on the auto-invoke path the harness resolves the pin (`${CLAUDE_EFFORT}` in the skill body renders `low`) but the turn keeps the session effort. **Pending harness fix** (reported via `/bug`, 2026-09-26): `runtime-feature-verify` and `disk-reclaim` still carry high judgment inline; grep `PENDING-HARNESS-FIX` for the markers. When the bug is fixed, re-run the probe and delete the markers and this paragraph.
