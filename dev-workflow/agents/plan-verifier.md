...
| 有 Bug diagnosis 的计划 | 计划头部 `**Bug diagnosis:**` 字段非 `not applicable`（fix-bug 大修复交给 write-plan 时写入） | BD |
...
**前置条件**：计划头部 `**Bug diagnosis:**` 字段非 `not applicable`。该字段由 fix-bug 在大修复（多文件 / 架构性 / 替换设计）交给 write-plan 时写入。字段为 `not applicable` 时跳过本策略。字段值形如 `see .claude/bug-diagnosis-*.md` 时读取该文件；文件不存在 → must-revise。

**目的**：诊断出的问题要真的落到 task 上，不能只写进计划头部就没人管。诊断本身不重做，fix-bug 已经确认过原因。

逐条核对，下列每一项都要有 task 处理，或在 `## Decisions` 里有 `[DP-xxx]` 说明为何不处理：
1. **确认的原因**（带 file:line）：至少一个 task 改到该位置。
2. **同一原因的其它现场**：每一处都有 task 修。漏一处 = 用户报的那处修好了，同类的其它地方还是坏的。
3. **受影响的消费方**：每个消费方修复后读到的值，要和某个 task 的 `Expected behavior` 一致。
4. **替换设计时**：替换新带来的每个代价，要么有 task 处理，要么在计划里明写为接受的缺陷；「在什么条件下旧路径会回来」要写在计划头部或某个 DP 里。

任一项既没有 task 也没有 DP → **must-revise**，输出：

```
❌ BD Gap [must-revise]: {诊断条目} 无 plan task 覆盖
   修订：加 task 处理 {file:line}，或在 `## Decisions` 里说明为何不处理
```
