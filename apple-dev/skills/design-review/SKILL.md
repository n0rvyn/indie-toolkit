...
App 设计质量审查。关注视觉层级、色彩策略、间距节奏、整体打磨感。与 `ui-review` skill（代码合规）互补。

Preferred caller: usually invoked by `run-phase` for new pages, modules, or components.
...
- [ ] 标签 + 数值组合是否按 §20.2 折叠为一个语义单元（"Posted 2h ago" 而非 `Date: 2h ago`）？
- [ ] 字重与对比度是否按 §20.3 平衡（高对比色 → 较细字重；低对比色 → 较粗字重）？

### A2. 色彩策略

> 原则参考：`ui-design-principles.md` §3 颜色系统
> 注：颜色合规性（语义色、深色模式变体、对比度）归 `ui-review` skill；色彩策略（比例、和谐、种类数）归本命令。
...
- [ ] 文字叠图区域是否按 §18.2 的四种策略之一保证对比度（overlay scrim / colorize tint / contrast-down / text-shadow）？
...
- [ ] 卡片阴影是否使用 §17.2 五级 elevation 系统的 token（level 0–4），而非随手填的 (offset, radius, opacity)？
...
> 💡 完成设计审查后，如需完整 Token 合规扫描，run-phase review step 会自动调用 `validate-design-tokens` skill。
...
- [ ] 页面是否应用了至少一个 §19 收尾打磨手段（accent border §19.2 / 背景装饰 §19.3 / 自定义控件 §19.1），还是处处依赖系统默认外观？
...
4. **与 `ui-review` skill 不重复**：本命令不检查状态完整性、无障碍标签、交互防护等 `ui-review` 已覆盖的项目
