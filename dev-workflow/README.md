...
| afk | driver (manual entry, inherit) | **Goal-oriented** unattended driver — the counterpart to the plan-oriented `write-plan → verify-plan → execute-plan` path. Needs a goal, a runnable convergence signal, and a surface; **not** a plan file. Front-loads every human-required unblock (device auto-lock, wired transport, on-device trust dialog, sudo) while the user is still present, then runs. Stops only at: signal red twice, reaching outside the declared surface, an irreversible action, a difference only the user can judge, an ambiguous instruction, or work that turns out to need a plan. Two mechanical gates — a done-claim must carry freshly-run output, and no prose hand-back once the model already has an opinion |
