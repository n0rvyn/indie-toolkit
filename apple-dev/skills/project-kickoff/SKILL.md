...
description: "Manual /project-kickoff only; not model-invocable. Runs the project kickoff flow to clarify requirements, converge scope, and produce execution recommendations. Use when starting a new project on any platform (distinct from /write-dev-guide, which plans phased development after design is approved); iOS/macOS projects additionally receive Apple-native initialization (DesignSystem/ASC/CI-CD)."
disable-model-invocation: true
---

<!-- cost-posture: inherit (judgment/orchestration — 5 AskUserQuestion 检查点 + WebSearch 市场调研 + dispatch generate-design-prompt / generate-design-system / design-reviewer；多检查点编排，do-NOT-downgrade per project CLAUDE.md Skill Cost Posture 规则) -->
