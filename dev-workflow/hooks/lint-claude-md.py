...
ACKNOWLEDGED = (
    "⚠️", "尚未创建", "不存在", "not created", "does not exist",
    # An author who wrote "（如有）" or "（待生成）" already knows the path is not
    # there yet. Flagging those lines nags at correct writing, which is the noise
    # mode this linter is built to avoid.
    "如有", "待生成", "待创建", "待补", "尚未生成", "if present", "TBD", "planned",
)

# Check 3 targets INSTRUCTIONS to use a skill Claude cannot invoke. A sentence
# that merely records where the file came from is not an instruction:
#   "由 `/apple-dev:project-kickoff` skill 在 2026-05-07 立项时生成"
# Provenance is signalled right before the reference, so look only at the text
# immediately preceding it rather than the whole line.
PROVENANCE = ("由", "出自", "来自", "generated by", "created by", "produced by")
PROVENANCE_WINDOW = 14

# Nor is a line that already routes the skill to the user rather than to Claude.
# Without this the linter would nag forever on every line it caused to be fixed.
UNINVOCABLE_OK = (
    "提示用户运行", "让用户运行", "请用户运行", "由用户运行",
    "模型不能自行调用", "不能由模型调用", "disable-model-invocation",
    "prompt the user to run", "ask the user to run", "cannot invoke it",
)
...
def find_slash_refs(text):
    """(name, line_number, is_provenance) for every slash-command-looking token."""
    out = []
    for m in SLASH_RE.finditer(text):
        name = m.group(1) or m.group(2)
        if not name:
            continue
        before = text[max(0, m.start() - PROVENANCE_WINDOW):m.start()]
        prov = any(marker in before for marker in PROVENANCE)
...
def lint(path):
    ...
    for name, lineno, is_provenance in find_slash_refs(text):
        bare = name.split(":")[-1]
        ...
        elif index[bare] is False and not is_provenance:
            if any(ok in lines[lineno - 1] for ok in UNINVOCABLE_OK):
                continue
