...
This skill **distills what the user has already decided**. It does not surface new candidate decisions, propose AI-perceived structure, or ask the user to evaluate AI inferences. The crystal file must contain only user-traceable content.

**Test for any step, field, or future amendment**: does this let the user lock in a settled conclusion, or does it ask the user to process AI-generated material? If the latter, it does not belong here.

**Where AI inferences go**: if the AI notices a gap, an implied constraint, or an unstated linkage during extraction, it surfaces these *verbally* in Step 3 as suggestions ("I noticed X was not explicitly decided — should this be a [D-xxx]?"). The user decides whether to add them. AI inferences do not enter the file as a quarantined sub-section, do not auto-populate optional fields like `(linked: ...)` or `does NOT reject ...`, and do not generate Phase A/B headers.

**Why this matters**: this file is consumed by `/write-plan` and `/verify-plan` without sub-section discrimination — any AI inference left in the file gets treated as a settled decision downstream. Keeping the file 100% user-traceable preserves the integrity of the entire planning pipeline. Any future amendment that adds an AI-generation step (e.g., "AI suggests N more decisions for user review") violates this principle and should be rejected.
...
**Medium-signal sources** (less explicit but still real — extract into the same fields as high-signal; the Decision traceability rule below filters any genuine AI inferences out to Step 3 verbal):
...
- **{Alternative name}**: Rejected because — {specific reason}. Rejection scope: {scope}; does NOT reject {what remains valid}. {append the Rejection scope clause ONLY when the user explicitly distinguished it}

## Decisions (machine-readable)
- [D-001] {clear conclusion in imperative form, traceable to user's words}
- [D-002] {clear conclusion} (linked: D-003 — {relationship description, only if user-stated})
...
- **Decision traceability**: Each `[D-xxx]` must be decidable (a plan can be checked against it) AND traceable to a specific user statement. For each D-xxx, the AI must be able to point to the user's words that led to it. If the AI cannot quote or closely paraphrase the user, it is an AI inference — **do NOT enter it into the file** (not as a D-xxx, not as a separate sub-section, not anywhere). Per Core Principle: AI inferences are raised verbally in Step 3 as suggestions; only items the user accepts become primary D-xxx.
- **Decision relationships**: Record `(linked: D-006 — {reason})` ONLY when the user explicitly stated the linkage ("A and B should be in the same module", "if A is strict enough, B is unnecessary"). If the linkage is AI-perceived but not user-stated, do not record it inline — raise it in Step 3 as a verbal suggestion instead. Do not split user-linked decisions into independent items without preserving the linkage.
...
- **Rejected Alternative precision**: Required format: `**{Alternative name}**: Rejected because — {reason}`. Optional: append `Rejection scope: {scope}; does NOT reject {what remains valid}` ONLY when the user explicitly distinguished what remains valid (e.g., user said "I'm rejecting the whole Method C design, but Bash fine-grained management as a direction is still open"). Do not infer the rejection boundary; if the user did not distinguish, write only the reason. Record the rejected item at the granularity the user actually rejected — if the user rejected a whole approach, do not split into component rejections; if the user rejected only a specific design within an approach, do not inflate to rejecting the approach.
- **Timeline awareness**: Use Phase headers (e.g., "Phase A: before X discovery", "Phase B: after X discovery") in Discussion Points ONLY when the user explicitly stated a pivot ("我之前想 X，后来发现 Y，现在改成 Z"). Do not auto-detect turning points — most discussions do not have phases, and AI-detected phases manufacture narrative structure the user has to verify. If user-stated phases exist, Rejected Alternatives may note which phase they belong to.
...
Present the extracted crystal to the user in markdown format.

**If during Step 2 the AI noticed any inference-worthy observation** (an implied constraint not explicitly stated, a possible linkage between decisions, a rejection scope the user did not distinguish, a possible discussion pivot the user did not flag), raise them *here, verbally*, before the three confirmation questions:

> Pending AI observations (not yet in the file):
> - I noticed {X} was discussed but not explicitly decided — should this be a [D-xxx]?
> - {D-002} and {D-005} look related to me — is there a linkage to record?
> - {Rejected alternative} — did you intend to also reject {sub-component Y}, or is Y still open?

The user replies item-by-item. Only items the user accepts get added as primary entries; the rest disappear. **Do NOT write these observations to the file unless the user accepts them** — per Core Principle, the crystal file stays 100% user-traceable.

**Do not generate observations to fill this block.** It exists only to surface real observations you naturally formed during Step 2 extraction. If you did not form any observations, skip the block entirely — do not invent them to seem thorough or to match the example bullets above. A clean Step 2 extraction with zero pending observations is the expected default, not a failure.
