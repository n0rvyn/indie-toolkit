...
| severe failure / failed structural re-verify / cannot-proceed terminal / seam-done | card **+ doc** | card **+ doc** |
...
| **Plan fails verification** | `verify-plan` returns `must-revise` | **CONTINUE** — apply the revision items to the **plan text only** (never source files) and log them to the run log. Re-verify once only if the revision changed the plan's structure (verify-plan Step 3's one-round contract). **STOP** only when a revision item requires a `blocking` decision (that DP takes the blocking row above), or a structural re-verify still returns `must-revise`. |
...
1. **Plan (guide mode, phase without a plan only):** write the phase's plan via the Plan Writing Reference, then `verify-plan`. Skip run-phase's scope-confirmation pause. A `must-revise` takes the **Plan fails verification** row: apply the revision items to the plan text and log them; re-verify once only after a structural change. Only a revision item that needs a `blocking` decision, or a structural re-verify that stays `must-revise`, escalates to STOP (writes handoff card, ends turn).
...
6. **Test + review (the per-unit quality gate, runs in BOTH modes):** `dev-workflow:test-changes` — a build/test/lint failure takes the **First red on a check** row (one byte-identical re-run; still red → severe → STOP with card). Then **one call to `dev-workflow:review-execution`**, passing `plan_path` (this unit's plan), `scope_files` (the plan's declared `**Files:**`), and `mode: gated`. Do not keep a reviewer list here — routing lives in that skill, and a second list is how the same agents ended up dispatched twice by different signals. Triage each finding:
...
- **Severity gates are never suppressed** — only pacing pauses are. A blocking DP and an explicit `<!-- checkpoint -->` stop immediately, in both modes. Three gates stop only *after* their bounded loop is exhausted, and the exhausted loop is itself the gate — suppressing the gate is still forbidden, shortening the loop is not the same thing:
  ...
  |---|---|---|
  | `must-revise` from verify-plan | revise plan text; re-verify once only after a structural change | a structural re-verify still `must-revise`, or an item needs a blocking decision |
