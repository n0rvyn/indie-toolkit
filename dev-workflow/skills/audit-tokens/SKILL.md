...
- Scans installed plugin SKILL.md files for cost-posture findings: inline `model:` pins without `context: fork` (no-op on auto-invoke), and lookup/tool-wrapper skills running inline with ≥$5 of main-model usage (fork candidates)
...
- **Pricing assumption**: uses public Anthropic list pricing as of 2026-09 — Opus 5.5 ($4/$20 per 1M in/out, $0.20 cache read) / Sonnet 5 ($2/$10, $0.20 cache read) / Haiku 4.5 / Fable 5.1. Older model generations are priced at these rates too, so totals are a normalized ranking. If the user is on a flat-rate plan, the dollar numbers are notional (treat as relative ranking, not actual billing).
