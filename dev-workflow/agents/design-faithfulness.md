...
### [DF-0] 契约自洽 precheck —— 先验基准，再拿基准去判别人
...
> **为什么这一步必须存在。**
> 本策略原本把设计文档当作**无误的真值**：它逐条比对「设计要求 → 计划有没有覆盖」，却从不问「这条设计要求本身立不立得住」。
>
> 一份实测过的真实契约里：6 处引用指向一个**从未被写出来的章节**；头注释宣传一个**不存在的类型**；自称 lockstep 的 token 镜像**漏掉 12 个 token**；**整份文档没有暗色 ink 阶梯**。
>
> 拿这样一份基准去判计划，会发生两件事，都是错的：
> - 计划**没**覆盖那个不存在的章节 → 被判「不忠实」→ 开发者去实现一个不存在的东西
> - 计划**忠实地**继承了「没有暗色阶梯」这个缺陷 → 被判「忠实」→ 缺陷带着一张合格证出货
>
> **一个把坏基准当好基准的验证器，比没有验证器更糟：它给缺陷发合格证。**
