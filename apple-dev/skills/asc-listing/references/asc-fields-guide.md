...
| 字段 | 类型 | 限制 | 填写说明 |
|------|------|------|----------|
| **Name** | 文本 | 最多 30 字符 | App 在 App Store 显示的名称。搜索权重最高的字段。⚠️ 不得含他人品牌（Review Guidelines 4.1(c)），且改名要走完整审核周期 |
| **Subtitle** | 文本 | 最多 30 字符 | 名称下方的副标题。第二权重字段。第三方品牌词放这里，用「for X」指代式 |

> 这三个字段（Name / Subtitle / Keywords）填什么文字，用 `/aso-research` 从实拉的排名与竞品数据定，不要凭感觉写。
