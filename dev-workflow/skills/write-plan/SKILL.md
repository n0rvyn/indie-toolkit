...
12. **Plan-time test-impl split — opt-in** (per `dev-workflow/references/tdd-research-2026.md` § 2026 mitigation stack subsection on plan-time test-impl split). **Default: one combined task.** Split only when the user asks for test isolation for this plan (then set `**Test isolation:** split` in the plan header). Why opt-in: across 292 labelled execute-plan agents, splitting turned 89 tasks into 178 agents, and every agent re-reads the plan; no measurement showed the split catching what a combined task missed. When the split is requested, a conceptual task that produces BOTH test files AND non-test files **containing executable logic** (source code, not config/data) becomes TWO tasks:
