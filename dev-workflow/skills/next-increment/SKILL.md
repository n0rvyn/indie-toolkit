...
When candidates are close on archetype + signal velocity + complexity, use the module-shape lens (see `${CLAUDE_PLUGIN_ROOT}/references/deep-modules-pattern.md`) as a tiebreaker: prefer the candidate that creates a deep module / a clean seam over one that adds an adapter or a shallow boundary. Cite the seam-vs-adapter or locality reasoning in the `Reasoned top-1` one-sentence rationale.
...
- **Deletion-test mandatory.** Candidates that fail the deletion test (work whose absence wouldn't change observable behavior) are rejected; regenerate. See `${CLAUDE_PLUGIN_ROOT}/references/deep-modules-pattern.md` for the deletion-test definition.
