...
> 未触发（步骤 2.5 触发门四条全不中）时，本章节只写一行：「无机制依赖，跳过领域研究检索」，删除下面的表格。

### 机制验证

> 字段与 SKILL.md 步骤 2.5.6 的输出格式一一对应，那里是唯一来源；改字段要两处同步。

| 机制 | 支撑功能 | 核心 | 状态 | 结论 | 来源 | 迁移性 |
|------|---------|------|------|------|------|--------|
| ... | ... | 是/否 | ✅/❌/⚠️ | ... | {摘要页 URL} | 直接适用 / 仅作背景 |

> 「核心」= 它不成立则产品不成立（决定 ❌ 条目是否停流程）。
> 「迁移性」= 该来源测的机制与本项目的机制是否同一件事。标「仅作背景」的条目不得用于定阈值、定准确率预期、或论证护城河。**填写范围：✅ / ❌ / ⚠️无定论 三种必填**——❌ 尤其不能省，拿一份测了别的机制的研究毙掉一个方向，是这一栏要拦的主要事故。**检索无命中 / 抓不到一手来源 / 一手来源抽验未通过 三种留空**，因为手上没有可比对的来源，硬填就是编。
> ⚠️ 的四种子状态要写明是哪一种，不要合并成一个「无定论」：无定论（文献确实有分歧）/ 检索无命中（没搜到，正控已验过检索器）/ 抓不到一手来源 / 一手来源抽验未通过。
...
- [{标题}]({摘要页 URL}) - {一句话结论} - 迁移性：直接适用 / 仅作背景
- [{库名}]({仓库 URL}) - {做什么} - {许可证}

> 每条都必须是 WebFetch 实际抓到过的页面。凭记忆写的引用一律不进本节。
```
