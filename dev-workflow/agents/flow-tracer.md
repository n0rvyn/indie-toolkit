...
| `field-never-written` | A state-bearing property is **declared and read** but Grep finds **zero writes** to it. For SwiftUI: for each `@State` / `@Published` / `@Observable var X`, search the module for `X = ` (not `==`), `$X` (binding), `X.toggle()`, `X.append`, `X.removeAll`. **Zero hits → the property is a constant wearing a state annotation.** The mirror image of `field-never-read`, and the more dangerous one: every branch that reads it is written, compiles, and renders — only one of them can ever be reached. A four-state enum with all four branches built and a `@State` that is never assigned looks *complete* to a screenshot, to a render-diff, and to a "did you handle all the states?" checklist. It is dead. Reference implementation: `n2_dead_state.py` (design-detectors — apple-dev plugin `scripts/design-detectors/`; handed-off repos carry a vendored copy at `scripts/design-gates/`) |
