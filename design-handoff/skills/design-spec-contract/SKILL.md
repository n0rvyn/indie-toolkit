...
| | Run it when? | Why |
|---|---|---|
| **Contract lint** — `python3 apple-dev/scripts/design-detectors/n4_contract_lint.py <contract-dir>` (dangling anchors · ghost symbols · mirror set-difference · light+dark ladder completeness) | **Always. Before every handoff, solo included.** `handoff-manifest` Step 1b gates on it. | It takes seconds and it is the **only** check the contract ever receives. Everything downstream — reviewers, drift auditors, the build agent — treats the contract as ground truth and never questions it. A defect that ships here is never caught again. |
...
> **What leaks is a value, not a file.** A value has no filename to recognise it by. The only thing that stops it is naming it here — and a co-occurrence grep downstream (`apple-dev/scripts/design-detectors/n3_scaffold_leak.py`: `.ignoresSafeArea()` on a content container **∧** a literal edge padding ≥ 48 → 🔴).
