...
from common import (
    ...
    Block, camel_tokens, collect_sources, line_of, rel, scrub,
...
def check_arm(arm: str, rule: Rule) -> list[Finding]:
    ...
    for path in collect_sources(arm):
...
def main() -> int:
    ...
    # A detector that cannot find its target or its ruler must say so — exit 2
    ...
    for arm in arms:
        if not collect_sources(arm):
            ...
            return 2
    if not a.contract:
        print(f"{RED}✖ no contract given{RESET}", file=sys.stderr)
        print("  N1 compiles its assertions FROM the contract's ## Platform Mapping table.", file=sys.stderr)
        print("  Pass --contract <dir-holding-DESIGN.md>.", file=sys.stderr)
        print("  (exit 2 = the check did not run. This is NOT a pass.)", file=sys.stderr)
        return 2
