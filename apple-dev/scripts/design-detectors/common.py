...
def repo_root() -> FSPath:
    return FSPath(__file__).resolve().parent.parent


def arm_dir(arm: str) -> FSPath:
    return repo_root() / "run2" / "arms" / arm


def arm_sources(arm: str) -> list[FSPath]:
    d = arm_dir(arm) / "Sources"
    return sorted(d.glob("*.swift"))


def rel(p: FSPath) -> str:
    """Path relative to the repo root, for citations."""
    try:
