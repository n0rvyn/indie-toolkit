...
⛔ **Never use `XCUIDevice.shared.appearance` on a real device.** It changes the **user's own system setting**, and there is no way to put it back: the API accepts only `.light` / `.dark`, and `.unspecified` throws at runtime (`Failed to set appearance mode: Requested appearance value (0) is not valid` — measured, iPhone 16 Pro / iOS 27 / Xcode 26.3). It is also write-only, so the harness cannot even learn what the setting was before it clobbered it. A run that leaves someone's phone forced to light is worse than a run that never happened.
- Use the env-gated `.preferredColorScheme` hook above instead — it changes only the app, never the device.
- If a harness already calls `XCUIDevice.shared.appearance` and you cannot remove it, it MUST capture-then-restore: launch the app once under a `-uiReportAppearance`-style flag that (a) suppresses the app's own color-scheme override and (b) exposes `@Environment(\.colorScheme)` as an accessibility identifier the test can read; store that value; restore it in `tearDown`. Both halves matter — without (a) you read back your own override and "restore" to the wrong mode. Never guess a default in `tearDown`; if the value could not be read, leave the device alone and fail the run.
