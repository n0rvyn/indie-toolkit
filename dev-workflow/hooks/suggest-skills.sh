...
if echo "$lower" | grep -qE 'commit|提交|save progress'; then
  ...
  if ls ~/.claude/plugins/cache/*/apple-dev/ >/dev/null 2>&1; then
    echo "[skill-hint] Related: /project-kickoff — guided iOS project initialization (apple-dev plugin)"
  fi
...
if git rev-parse HEAD >/dev/null 2>&1; then
  _claude_md_recent=$(git log -n 10 --pretty=format:"%H" -- CLAUDE.md 2>/dev/null | wc -l | tr -d ' ')
  if [ -n "$_claude_md_recent" ] && [ "$_claude_md_recent" -ge 1 ]; then
    echo "[skill-hint] CLAUDE.md 在最近 ${_claude_md_recent} 个 commit 内被修改 — 考虑 /audit-rules 检查规则冲突" >&2
  fi
fi

# Finish-branch nudge: suggest /finish-branch when feature branch ahead of main
if git rev-parse HEAD >/dev/null 2>&1; then
  ...
  if [ -n "$_current_branch" ] && [ "$_current_branch" != "$_main_branch" ] && [ "$_current_branch" != "main" ] && [ "$_current_branch" != "master" ]; then
    _ahead=$(git rev-list --count "$_main_branch..HEAD" 2>/dev/null)
    if [ -n "$_ahead" ] && [ "$_ahead" -ge 5 ]; then
      echo "[skill-hint] 当前分支 ${_current_branch} 领先 ${_main_branch} ${_ahead} 个 commit — 考虑 /finish-branch 完成集成" >&2
    fi
  fi
fi

# audit-tokens cadence nudge: suggest /audit-tokens every 14 days
if git rev-parse HEAD >/dev/null 2>&1; then
  _audit_tokens_state="$HOME/.claude/last-audit-tokens-run"
  _audit_tokens_days_threshold=14
  if [ ! -f "$_audit_tokens_state" ]; then
    echo "[skill-hint] 未发现 /audit-tokens 运行记录 — 考虑跑一次了解 token 消耗" >&2
  else
    _audit_tokens_age_days=$(( ($(date +%s) - $(stat -f %m "$_audit_tokens_state" 2>/dev/null || stat -c %Y "$_audit_tokens_state" 2>/dev/null)) / 86400 ))
    if [ "$_audit_tokens_age_days" -ge "$_audit_tokens_days_threshold" ]; then
      echo "[skill-hint] 距上次 /audit-tokens 已 ${_audit_tokens_age_days} 天 — 考虑再跑一次" >&2
    fi
  fi
fi
