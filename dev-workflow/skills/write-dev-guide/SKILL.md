...
5. **Project Context Contract** — read `${CLAUDE_PLUGIN_ROOT}/references/project-context-contract.md`; if `docs/00-AI-CONTEXT.md` exists, read it as product language, module map, and validation command source. If missing, continue and mark `Project context contract: missing`. Do not create `CONTEXT.md`.
6. **Project Health** — if `${CLAUDE_PLUGIN_ROOT}/scripts/project_health_scan.py` exists, read `.claude/dev-workflow-health.json` first; if state is missing/red-signal/older-than-7-days, run scanner full mode with `--reason dev-guide --check-staleness 7 --max-ms 5000 --write-state`. Otherwise use cached `last_health`. Use red/yellow signals as guide constraints or decision candidates.
